...
export class AppointmentDateRangeComponent {
  ...
  onDateSelection(date: NgbDate) {
    ...
    if (this.fromDate && this.toDate) {
      ...
      this.dateRange = {
        from: new Date(this.fromDate.year, this.fromDate.month - 1, this.fromDate.day).toISOString(),
        to: new Date(this.toDate.year, this.toDate.month - 1, this.toDate.day).toISOString()
      };
